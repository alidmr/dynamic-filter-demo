    ...
    $('#ProductGrid').DataTable({
        ...
        ajax: {
            ...
            data: function (data) {
                ...
                $.each(formItems, function (index, element) {
                    if (this.value != undefined && this.value != '' && this.value != 'select') {
                        if (!this.name.includes('_ConditionType')) {
                            debugger;
                            var checkItem = filterList.filter(x => x.propertyName === this.name);
                            if (checkItem.length > 0) {
                                var checkItem2 = (element) => element.propertyName === this.name;

                                var checkItemIndex = filterList.findIndex(checkItem2);

                                var itemValue = '';
                                itemValue += filterList[checkItemIndex].value;

                                //delete filterList[checkItemIndex];

                                filterList = arrayRemove(filterList, filterList[checkItemIndex]);

                                itemValue += ',' + this.value;

                                var filterItem = {
                                    'propertyName': this.name,
                                    'operatorType': $('#' + this.name + '_ConditionType').val(),
                                    'value': itemValue
                                };
        ...
        columns: [
            ...
            { data: 'categoryName', name: 'CategoryName', 'orderable': true },
...
function ChangeActiveOrPassiveItem(itemId, isActive) {
    ...
    Notiflix.Confirm.Show('Emin misiniz?', message, 'Evet', 'Hayır',
        function () {
            return;
            $.ajax({
                url: '/Product/ChangeActiveOrPassiveItem',
                method: 'POST',
                data: { 'itemId': itemId }
            }).done(function (response) {
                if (response.success) {
